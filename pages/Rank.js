document.addEventListener('DOMContentLoaded', function () {
    const api = '/api/getUserRank';
    //fetch(api).then(data => {
    let data = {
        "rankings": [
            {
                "group": "UserGroup", "name": "Test99", "score": 100, "data": {
                    "max_temp": 33,
                    "min_temp": 20,
                    "wind_speed": 5,
                    "precipitation": 2.3
                }
            },
            {
                "group": "UserGroup", "name": "Test999", "score": 90, "data": {
                    "max_temp": 33,
                    "min_temp": 20,
                    "wind_speed": 5,
                    "precipitation": 2.3
                }
            },
            {
                "group": "UserGroup", "name": "Test3", "score": 10, "data": {
                    "max_temp": 33,
                    "min_temp": 20,
                    "wind_speed": 5,
                    "precipitation": 2.3
                }
            },
            {
                "group": "UserGroup", "name": "Test2", "score": 20, "data": {
                    "max_temp": 33,
                    "min_temp": 20,
                    "wind_speed": 5,
                    "precipitation": 2.3
                }
            },
            {
                "group": "UserGroup", "name": "Test1", "score": 30, "data": {
                    "max_temp": 33,
                    "min_temp": 20,
                    "wind_speed": 5,
                    "precipitation": 2.3
                }
            },
            {
                "group": "UserGroup", "name": "预报1", "score": 40, "data": {
                    "max_temp": 33,
                    "min_temp": 20,
                    "wind_speed": 5,
                    "precipitation": 2.3
                }
            },
            {
                "group": "UserGroup",
                "name": "预报2", "score": 50, "data": {
                    "max_temp": 33,
                    "min_temp": 20,
                    "wind_speed": 5,
                    "precipitation": 2.3
                }
            }
        ]
    }
    const rankingList = document.getElementById('ranking-list');
    rankingList.innerHTML = '';
    const sortedRankings = data.rankings.sort((a, b) => a.score - b.score);
    sortedRankings.forEach((item, index) => {
        const row = document.createElement('tr');
        const rankIcon = index < 3 ? '✨' : '';

        row.innerHTML = `
<td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">${rankIcon} ${index + 1}</td>

<td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm sm:pl-6">
<div class="flex items-center">
    <div class="h-10 w-10 flex-shrink-0">
        <img class="h-10 w-10 rounded-full"
            src="data:image/png;base64,"
            alt="" />
    </div>
    <div class="ml-4">
        <div class="text-[medium] font-medium text-gray-900">${item.name}</div>
        <div class="text-gray-500">${item.group}</div>
    </div>
</div>
</td>
<td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
<span class="inline-flex rounded-md bg-[#16a34a33] px-2 text-xs font-[500] leading-6 text-green-800">Max_T:
    ${item.data.max_temp}°C Min_T: ${item.data.min_temp}°C Wind: ${item.data.wind_speed}m/s Rain24:
    ${item.data.precipitation}mm</span>
</td>
<td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 text-right">${item.score}</td>`;

        rankingList.appendChild(row);
    });
    /*})
        .catch(error => {
            console.error(error);
            const rankingList = document.getElementById('ranking-list');
            rankingList.innerHTML = '<li>无数据</li>';
        });*/
    let data_date = "站点1 1926-08-17"
    let data_obs = "昨日实况：最高气温：1°C；最低气温：1°C；风：1m/s；24H降水：1mm"
    document.getElementById('date').textContent = `${data_date}`;
    document.getElementById('obs').textContent = `${data_obs}`;
});